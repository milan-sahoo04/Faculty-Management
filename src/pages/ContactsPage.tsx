...
// --- Dummy Data (Indian Faculty/Staff - Same as yours) ---
...
const ContactAvatar: React.FC<{ name: string }> = ({ name }) => {
  ...
  return (
    <div
      className={`h-14 w-14 rounded-full flex items-center justify-center font-bold text-white text-xl ${bgColor} flex-shrink-0 border-2 border-white dark:border-gray-800 ring-2 ring-indigo-300 dark:ring-indigo-700`}
    >
      {initials}
    </div>
  );
...
// The Contact Card component (UI Enhancement + Dark Mode)
const ContactCard: React.FC<{ contact: Contact }> = ({ contact }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl border-l-4 border-indigo-500 p-6 flex flex-col transition-all duration-300 hover:shadow-2xl hover:bg-indigo-50 dark:hover:bg-gray-700">
      <div className="flex items-center pb-4 mb-4">
        ...
        <div className="ml-4 flex-1">
          ...
          <p className="text-sm text-indigo-700 dark:text-indigo-400 font-medium flex items-center mt-1">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center text-xs mb-4 gap-2 sm:gap-0">
        <span className="px-3 py-1 rounded-full font-semibold text-gray-700 dark:text-gray-300 bg-gray-200 dark:bg-gray-700 w-fit">
          {contact.department}
        </span>
        <span
          className={`inline-flex items-center px-3 py-1 rounded-full font-bold w-fit ${
            contact.isAvailable
      ...
      <div className="space-y-2 text-sm text-gray-700 dark:text-gray-300 border-t border-gray-100 dark:border-gray-700 pt-4">
        <p className="flex items-center">
          <Mail className="h-4 w-4 mr-2 text-blue-500" />
          <a
            href={`mailto:${contact.email}`}
            className="text-blue-600 dark:text-blue-400 hover:text-blue-800 transition truncate"
          >
            {contact.email}
          </a>
        </p>
      ...
      <div className="mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
        <div className="flex flex-wrap gap-2">
          ...
          <button
            className="flex items-center px-4 py-2 text-xs font-medium text-indigo-600 bg-indigo-100 rounded-lg shadow-md hover:bg-indigo-200 transition dark:bg-indigo-900 dark:text-indigo-300 dark:hover:bg-indigo-800"
            onClick={() =>
        ...
        <div className="mt-4 pt-3 border-t border-gray-100 dark:border-gray-700">
          ...
          <div className="flex flex-wrap gap-1.5">
            {contact.researchInterests.map((interest, index) => (
              <span
                key={index}
                className="px-3 py-1 text-xs font-medium bg-gray-200 text-gray-700 rounded-full hover:bg-gray-300 transition dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600"
              >
                {interest}
              </span>
            ))}
...
const ContactPage: React.FC = () => {
  ...
  return (
    // 💡 Responsive and Dark Mode Container
    <div className="p-4 md:p-8 max-w-7xl mx-auto font-sans bg-gray-50 dark:bg-gray-900 min-h-screen">
      {/* Header */}
      <header className="mb-8 p-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg border-b-4 border-indigo-600">
        <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white flex items-center sm:text-3xl md:text-4xl">
          <Users className="h-5 w-5 sm:h-6 sm:w-6 md:h-8 md:w-8 mr-4 text-indigo-600 flex-shrink-0" />
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg mb-8">
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 border-2 border-gray-300 dark:border-gray-600 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 text-base shadow-sm transition bg-white dark:bg-gray-700 dark:text-white dark:placeholder-gray-400"
          />
        </div>

        {/* Filters Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 border-t border-gray-100 dark:border-gray-700 pt-4">
          {/* Department Filter */}
          <div>
            ...
            <select
              ...
              className="w-full border-gray-300 dark:border-gray-600 rounded-lg shadow-sm p-3 text-sm focus:ring-indigo-500 focus:border-indigo-500 bg-gray-50 dark:bg-gray-700 dark:text-white transition"
            >
          ...
          <div className="flex items-end">
            <button
              ...
              className={`flex items-center justify-center px-4 py-3 rounded-lg font-semibold transition-all shadow-md w-full ${
                showFacultyOnly
                  ? "bg-indigo-600 text-white hover:bg-indigo-700"
                  : "bg-white text-indigo-600 border border-indigo-300 hover:bg-indigo-50 dark:bg-gray-700 dark:border-indigo-500 dark:hover:bg-gray-600 dark:text-indigo-400"
              }`}
            >
      ...
      {filteredContacts.length > 0 ? (
        ...
        // No Results Card
        <div className="p-8 sm:p-16 text-center bg-white dark:bg-gray-800 rounded-xl shadow-lg text-gray-500 dark:text-gray-400 border-2 border-dashed border-gray-300 dark:border-gray-700">
          <Search className="h-10 w-10 sm:h-12 sm:w-12 mx-auto mb-4 text-indigo-400" />
          <p className="font-bold text-lg sm:text-xl text-gray-800 dark:text-gray-200">
